...
router.post('/drafts', authenticate, (req, res, next) => {
  req.body = { ...req.body, status: 'draft' };
  next();
}, validate(createEventSchema), asyncHandler(eventController.createEvent));
router.post('/', authenticate, validate(createEventSchema), asyncHandler(eventController.createEvent));
router.get('/:id', optionalAuthenticate, asyncHandler(eventController.getEvent));
